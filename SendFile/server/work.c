//
// process the recv file data
// 
//

#include "work.h"

//gconn array storage the connection info;
int freeid = 0;
...
void recv_fileinfo(int sockfd)
{
    ...
    //create helo file,and map data into memory
    char filepath[100] = {0};
    ...
    //add connection into gconn array
...
void recv_filedata(int sockfd)
{
    ...
    //recv data and map into memory
    int remain_size = fhead.bs;      //file block ready to recv
    int  size = 0;                   //size of recv data a times
    while(remain_size > 0){
...
//create socket fd,listen client
